...
var visitor = new function () {
    ...
    var query = function () {
        if (!isAjax) {
            isAjax = true;
            cm.ajax("index.php?m=Index&c=First&a=getVisitors", {index: pageIndex++, size: pageSize}, function (msg) {
    ...
    var createItem = function (user) {
        ...
        name.innerHTML = user["username"] + " , " + age + " , " + cm.formatDate(new Date(user["created"]*1000), "yyyy-MM-dd HH:mm:ss");
            ...
            if (menu == null) {
                var handler = [
                    function () {
                        ...
                        cm.ajax("index.php?m=Index&c=First&a=delVisitors", {
                            visitorId: user["id"]
                        }, function (res) {
