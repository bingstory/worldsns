...
class ConsumeModel extends Model {
	...
	public function showList($search,$pages) {
		...
		$list = $this->field(array('a.*','b.id'=>'b_id','b.username'=>'b_username','c.id'=>'c_id','c.username'=>'c_username','d.id'=>'d_id','d.name'=>'d_name','d.cate'=>'d_cate'))
			->alias('a')
			->join('user b ON b.id = a.sender_id')
			->join('user c ON c.id = a.reciver_id')
			->join('gift d ON d.id = a.gift_id')
