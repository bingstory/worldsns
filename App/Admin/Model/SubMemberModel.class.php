...
class SubMemberModel extends Model {
	...
	public function showList($search,$pages,$employeeId) {
		$userEmployeeModel = M('inner_member');
		$userIds = $userEmployeeModel->field('user_id')->where('job_number='.$employeeId)->select();
		$ids = array();
		...
		if (isset($search['inner_member']) && $search['inner_member'] != 'all') {
			if ($search['inner_member'] == 'yes') {
				$where['inner_member'] = true;
			}
			if ($search['inner_member'] == 'no') {
				$where['inner_member'] = false;
			}
		}
